...
export default function ExplorerGlobe({ targetRotY = 0 }) {
    const containerRef = useRef(null);
    const sphereRef = useRef(null);
    const currentRotYRef = useRef(0);

    // Store targetRotY in a ref so the animation loop can read it
    ...
    useEffect(() => {
        ...
        const uniforms = {
            ...
            colorForest: { value: new THREE.Color('#2A5A42') },
            colorInk: { value: new THREE.Color('#121212') },
        };

        const geometry = new THREE.IcosahedronGeometry(3, 40);
        ...
        const animate = () => {
            ...
            currentRotYRef.current += (targetRotYRef.current - currentRotYRef.current) * 0.05;
            sphere.rotation.y = currentRotYRef.current;
    ...
    return <div className="explorer-canvas-container" ref={containerRef} />;
}
